...
Sea = function(){
  var geom = new THREE.CylinderGeometry(game.seaRadius,game.seaRadius,game.seaLength,10,10);
  ...
  for (var i=0;i<l;i++){
    ...
    var m = new THREE.Mesh(geom.clone(), 0);
    m.position.x = v.x
    m.position.y = v.y;
    m.position.z = v.z
...
function collision (pos) {
  //test
  var diffPos = airplane.mesh.position.clone().sub(pos);
  var d = diffPos.length();
  if (d<game.ennemyDistanceTolerance){
    particlesHolder.spawnParticles(pos, 15, Colors.red, 3);
